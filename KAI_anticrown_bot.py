...
def Set_up_event(message):
    ...
    if user_Task == '147685':
        ...
        bot.send_message(message.chat.id, 'Некорректный айди ' + user_Task , reply_markup=event_keyboard)
    global sql_task 
    global sql_id
    global sql_date
    sql_date = today
...
def send_text(message):
    if message.text.lower() == 'зарегистрировать мероприятие':
        mci = message.chat.id
        New_event = bot.send_message(mci,'Регистрируем новое посещение', reply_markup=event_keyboard)
        ...
        bot.send_message(message.chat.id,'некорректный айди', reply_markup=start_button)
